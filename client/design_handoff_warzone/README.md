...
src/maps/MapLoader.js   loads a map JSON into a scene (paints + lane paths + buildable)
tools/forge/            sprite generator (00-core palette+helpers, 10..50 draw fns,
                        generate.html = in-browser re-bake → downloadable zip)
tools/maps/             maps-src.js (specs + symmetry) + generate.html (preview/zip)
public/assets/          the art + manifest.json
public/maps/            <id>.json map data + previews/<id>.png
```
