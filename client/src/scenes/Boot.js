...
export default class Boot extends Phaser.Scene {
  ...
  preload() {
    ...
    // map JSON (id comes from the initial server snapshot)
    const snapshot = this.registry.get('snapshot');
    const mapData = this.registry.get('mapData');
    const mapId = snapshot?.map?.id ?? 'twin_fronts';
    if (mapData) this.cache.json.add('map_data', mapData);
    else this.load.json('map_data', P(`maps/${mapId}.json`));
